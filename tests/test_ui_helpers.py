...
def test_card_actions_use_guarded_clicks_and_no_confirm_state() -> None:
    ...
    assert 'labels.push(`${counts.expired} 待确认`)' not in javascript
    ...
    assert '["expired", "待确认"]' not in javascript
    assert '["ended", "已结束"]' in javascript
    assert '["offered", "已 Offer"]' in javascript
    assert 'return ["offered", "ended"].includes(current) ? current : "active";' in javascript
    ...
    assert ".progress-company-state.attention" not in stylesheet
