...
人工台账保存后使用 `jobmaildesk sync-ledger` 或组件“同步台账”刷新卡片；该操作只读取本地 Markdown，不连接邮箱。带 `<!-- jobmaildesk:application:... -->` 稳定标记的行可以修改企业、岗位、当前进展和下一步动作。隐藏申请使用“已归档”，不得依靠删除自动生成的进展卡片；直接删除台账行不会删除任务事实层。
