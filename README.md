...
设置页可以选择 QQ、网易 163、网易 126、Yeah、Gmail、Outlook，或填写自定义 IMAP。选择预设后，主机、端口和 SSL/TLS 仍可修改。配置时填写：

- 完整邮箱地址；
- 邮箱服务商生成的客户端授权码或应用专用密码，**不要填写日常登录密码**；
- 扫描间隔，建议保留默认 10 分钟；
- 首次启动自动回看最近 30 天；完成后按设置中的短窗口扫描，默认 3 天。

QQ 与网易邮箱通常需要先在网页版设置中开启 IMAP 并生成客户端授权码；Gmail 通常需要应用专用密码。部分 Outlook 账户已经禁用传统账号密码 IMAP 登录，当前版本尚未提供 OAuth，因此不能保证所有 Outlook 账户都能直接连接。
...
完成操作会同步本地任务、Obsidian 待办总览和自动生成的求职进展。若设置了人工岗位台账，程序只在目标行能够唯一确认时更新“当前进展”字段并绑定稳定申请 ID；匹配不唯一时保持台账不变，避免串改同一企业的其他岗位。修改台账后可点击“同步台账”，只读取本地 Markdown，不扫描邮箱。

“待处理”是待确认与待归属的统一入口。无法唯一归属的邮件会按模板把脱敏结果分栏显示为公司、岗位、招聘项目/年份、阶段、轮次、开始、结束、截止、模糊时间提示和下一步行动，便于逐项核对：

- **保存草稿**只更新本地待处理记录，字段可以暂时留空，不会提前创建正式任务或申请链；
- **确认并入链**可以明确选择候选申请链，也可以用已经确认的公司和岗位新建申请链；
- 归入已有申请链时，该链锁定的公司、岗位和招聘项目是身份依据，卡片中残留或误改的身份文字不会污染其他岗位；
- “预计 8 月启动”等模糊提示会原样保留，但没有明确日期和时间时仍停留在待确认/进展中，不会虚构日历事项；
- 候选链快捷按钮和二次确认忽略仍然可用，重复扫描不会重新打开已经确认或忽略的同一邮件；
- 消息去重标识只在程序内部使用，界面不会显示哈希，也不要求用户复制或输入内部标识。

未完成归属前，邮件不会进入正式申请链、日历或研究队列。
...
- 规范行格式为 `公司｜岗位｜当前进展｜下一步动作 <!-- jobmaildesk:application:稳定申请ID -->`；组件状态同步只修改第三段，第四段及其他手写内容始终保留。
- 将台账状态改为“已归档”会关闭申请，不删除本地任务历史；直接删除台账行不会删除任务，没有稳定 ID 的行不会被自动删除。
