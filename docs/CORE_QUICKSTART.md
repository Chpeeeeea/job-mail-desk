...
1. Windows 下载 `JobMailDesk-Core-v0.6.0-win-x64.zip`；Apple Silicon Mac 下载 `macos-arm64`，Intel Mac 下载 `macos-x64`。
...
1. 选择 QQ、网易 163、网易 126、Yeah、Gmail、Outlook 或自定义 IMAP。
2. 填写完整邮箱地址与客户端授权码或应用专用密码，不要填写日常登录密码。
3. 检查自动填写的 IMAP 主机、端口和 SSL/TLS；如服务商要求不同，可以手动修改。
4. 保持默认10分钟扫描，或选择5至120分钟。
5. 不使用Obsidian时关闭“同步到Obsidian”。
6. 使用Obsidian时选择一个 `.md` 文件作为待办总览。
7. 如需手动维护企业进展，开启“生成求职进展文档”，并点击“生成规范台账模板”。
8. 先测试只读连接，再点击“保存并应用”。

QQ 与网易邮箱通常需要先在网页版设置中开启 IMAP 并生成客户端授权码；Gmail 通常需要应用专用密码。部分强制 OAuth 的 Outlook 账户暂不支持。
